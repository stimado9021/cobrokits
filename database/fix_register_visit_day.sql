-- Update register_customer_visit to validate visit_day matches customer's assigned day
...
BEGIN
  ...
  -- Validate visit_day matches customer's assigned day
  ...
  IF v_customer.visit_day IS NOT NULL AND v_customer.visit_day != v_visit_dow THEN
    RAISE EXCEPTION 'Este cliente solo puede ser visitado los días % (día asignado: %). Hoy es día %.', 
      v_customer.visit_day, v_customer.visit_day, v_visit_dow;
  END IF;
