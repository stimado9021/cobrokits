...
(async () => {
  ...
  // Check the visit_date column type now
  const col = await c.query("SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'customer_visits' AND column_name = 'visit_date'");
  console.log('visit_date column:', col.rows);
  
  // Check visits for today
  const today = new Intl.DateTimeFormat("en-CA", { timeZone: "America/Bogota" }).format(new Date());
  console.log('Today Colombia:', today);
  
  const visits = await c.query("SELECT id, visit_date, seller_id, new_products_total FROM customer_visits WHERE visit_date = $1::date ORDER BY created_at DESC", [today]);
  console.log('Visits for today:', visits.rows);
